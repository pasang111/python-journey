                                                # the below code is the code we need to debug
...
def validate_isbn(isbn, length):
    ...
    if length == 10:
        if not given_check_digit.isdigit() and given_check_digit != 'X':
            print('Invalid character was found.')
            return
    else:
...
def calculate_check_digit_10(main_digits_list):
    ...
    # Multiply each of the first 9 digits by its weight
    for index, digit in enumerate(main_digits_list):
        digits_sum += digit * (10 - index)
...
def calculate_check_digit_13(main_digits_list):
    ...
    # Multiply digits by 1 and 3 alternately
    for index, digit in enumerate(main_digits_list):
        if index % 2 == 0:
            digits_sum += digit * 1
        else:
...
def main():
    ...
    # Check if the user entered ISBN and length
    if len(values) != 2:
